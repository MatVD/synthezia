# Scriberr AI Coding Instructions
...
## Project Overview
Scriberr is a self-hosted audio transcription service combining a **Go backend** (Gin framework) with a **React/TypeScript frontend** (Vite), packaged as a single binary. It orchestrates Python-based transcription (WhisperX, NVIDIA models) via embedded Python environments using `uv` package manager.
...
5. Import in `cmd/server/main.go` (or rely on `_ "scriberr/internal/transcription/adapters"`)
